...
(function() {
    var AnimatedText, AnimatedTextFactory, AnimationUpdater, Bar, BaseDonut, BaseDonut2, BaseGauge, Donut, Gauge, GaugePointer, TextRenderer, ValueUpdater, addCommas, cutHex, formatNumber, mergeObjects, secondsToString,
        slice = [].slice,
    ...
    formatNumber = function() {
        ...
        return digits == 0 ? parseInt(value) : addCommas(value.toFixed(digits));
    };
    ...
    BaseGauge = (function(superClass) {
        ...
        BaseGauge.prototype.renderStartEndLabels = function(startEndLabels, w, h, radius) {
            var font, fontsize, labels, match, re, rest, origenRad, calcY;
            this.ctx.save();

            origenRad = ((1 - this.options.angle) * Math.PI) + Math.PI;
            calcY= Math.sin(origenRad) * radius;

            this.ctx.translate(w, h - calcY);
            ...
            if (labels[0] !== void 0) {
                this.ctx.fillText(labels[0] + ' ' + this.options.startEndLabels.unit, -radius - 15, fontsize - 5);
            } else { 
                this.ctx.fillText(parseInt(this.minValue) + ' ' + this.options.startEndLabels.unit, -radius - 15, fontsize - 5);
            }
            if (labels[1] !== void 0) {
                this.ctx.fillText(labels[1] + ' ' + this.options.startEndLabels.unit, radius + 15, fontsize - 5);
            } else {
                this.ctx.fillText(parseInt(this.maxValue) + ' ' + this.options.startEndLabels.unit, radius + 15, fontsize - 5);
            }
            return this.ctx.restore();
        };

        BaseGauge.prototype.renderStaticLabels = function(staticLabels, w, h, radius) {
    ...
    BaseDonut = (function(superClass) {
        ...
        BaseDonut.prototype.bottomTextValue = 0;
        ...
        BaseDonut.prototype.render = function() {
            var displayedAngle, grdFill, h, w, textHeight;
            displayedAngle = this.getAngle(this.displayedValue);
            w = this.canvas.width / 2;
            h = this.canvas.height / 2;

            this.ctx.shadowColor = "rgb(190, 190, 190)";
            ...
            if (this.options.icon) {
                this.ctx.font = this.options.icon.font;
                this.ctx.fillStyle = this.options.icon.fillStyle;
                this.ctx.textAlign = this.options.icon.textAlign;
                this.ctx.fillText(String.fromCharCode('0x' + this.options.icon.charCode), w, h);
            }

            if (this.textField) {
                textHeight = this.canvas.height - (this.canvas.height * 0.30);
                this.ctx.font = this.options.textField.font;
                this.ctx.fillStyle = this.options.textField.fillStyle;
                this.ctx.textAlign = this.options.textField.textAlign;
                this.ctx.fillText(formatNumber(this.displayedValue, this.options.textField.precision) + ' ' + this.options.textField.unit, w, textHeight);
            }

            if (this.options.bottomTextField) {
                textHeight = this.canvas.height - (this.canvas.height * 0.10);
                this.ctx.font = this.options.bottomTextField.font;
                this.ctx.fillStyle = this.options.bottomTextField.fillStyle;
                this.ctx.textAlign = this.options.bottomTextField.textAlign;
                this.ctx.fillText(formatNumber(this.bottomTextValue, this.options.bottomTextField.precision) + ' ' + this.options.bottomTextField.unit, w, textHeight);
            }
            
            if (this.options.startEndLabels) {
                this.renderStartEndLabels(this.options.startEndLabels, w, h, this.radius);
            }
    ...
    BaseDonut2 = (function(superClass) {
        extend(BaseDonut2, superClass);

        BaseDonut2.prototype.lineWidth = 15;

        BaseDonut2.prototype.displayedValue = 0;

        BaseDonut2.prototype.value = 0;

        BaseDonut2.prototype.bottomTextValue = 0;

        BaseDonut2.prototype.maxValue = 5000;

        BaseDonut2.prototype.minValue = -5000;

        BaseDonut2.prototype.setTextField = function(textField) {
            return this.textField = textField;
        };

        BaseDonut2.prototype.setMinValue = function(minValue, updateStartValue) {
            this.minValue = minValue;
            if (updateStartValue == null) {
                updateStartValue = true;
            }
        ...
        BaseDonut2.prototype.options = {
            lineWidth: 0.10,
            colorStartPositive: "#6f6ea0",
            colorStopPositive: "#c0c0db",
            colorStartNegative : "#6f6ea0",
            colorStopNegative: "#c0c0db",
            strokeColor: "#eeeeee",
            shadowColor: "#d5d5d5",
            angle: 0.35,
            radiusScale: 1.0
        ...
        function BaseDonut2(canvas) {
            this.canvas = canvas;
            BaseDonut2.__super__.constructor.call(this);
            if (typeof G_vmlCanvasManager !== 'undefined') {
                this.canvas = window.G_vmlCanvasManager.initElement(this.canvas);
            }
            this.ctx = this.canvas.getContext('2d');
        ...
        BaseDonut2.prototype.getAngle = function(value) {
            return (1 - this.options.angle) * Math.PI + ((value - this.minValue) / (this.maxValue - this.minValue)) * ((2 + this.options.angle) - (1 - this.options.angle)) * Math.PI;
        };

        BaseDonut2.prototype.setOptions = function(options) {
            if (options == null) {
                options = null;
            }
            BaseDonut2.__super__.setOptions.call(this, options);
            this.lineWidth = this.canvas.height * this.options.lineWidth;
            this.radius = this.options.radiusScale * (this.canvas.height / 2 - this.lineWidth / 2);
            return this;
        };

        BaseDonut2.prototype.set = function(value) {
            this.value = this.parseValue(value);
            if (this.value > this.maxValue) {
                if (this.options.limitMax) {
                    this.value = this.maxValue;
        ...
        BaseDonut2.prototype.render = function() {
            var displayedAngle, grdFill, h, w, textHeight;
            displayedAngle = this.getAngle(this.displayedValue);
            w = this.canvas.width / 2;
            h = this.canvas.height / 2;
            ...
            if (this.options.icon) {
                this.ctx.font = this.options.icon.font;
                if (this.options.icon.fillStyle == 'auto') {
                    var now = parseInt(this.bottomTextValue);
                    if (now >= 0 && now <= 40) {
                        this.ctx.fillStyle = 'red';
                    } else if (now >= 41 && now <= 80) {
                        this.ctx.fillStyle = 'yellow';
                    } else if (now >= 81 && now <= 100) {
                        this.ctx.fillStyle = 'green';
                    }
                } else { this.ctx.fillStyle = this.options.icon.fillStyle; }
                this.ctx.textAlign = this.options.icon.textAlign;
                this.ctx.fillText(String.fromCharCode('0x' + this.options.icon.charCode), w, h);
            }
            ...
            if (this.displayedValue < 0) {
                grdFill.addColorStop(0, this.options.colorStartNegative);
                grdFill.addColorStop(1, this.options.colorStopNegative);
            } else {
                grdFill.addColorStop(0, this.options.colorStartPositive);
                grdFill.addColorStop(1, this.options.colorStopPositive);
            }
            ...
            this.ctx.beginPath();        
            if (this.options.staticLabels) {
                this.renderStaticLabels(this.options.staticLabels, w, h, this.radius);
            }
            ...
            // Radius : 0.5 a 1.0 options.angle : -0.5 a 0.5 displayed angle : pi / 2 a (2 * pi) + (pi / 2) 
            ...
            var origen = (Math.PI / 2) + Math.PI;

            if (this.displayedValue < 0) {
                this.ctx.arc(w, h, this.radius, origen, displayedAngle, true);
            } else if (this.displayedValue > 0) {
                this.ctx.arc(w, h, this.radius, origen, displayedAngle, false);
            }
    ...
    Donut2 = (function(superClass) {
        extend(Donut2, superClass);

        function Donut2() {
            return Donut2.__super__.constructor.apply(this, arguments);
        }

        Donut2.prototype.strokeGradient = function(w, h, start, stop) {
            var grd;
            grd = this.ctx.createRadialGradient(w, h, start, w, h, stop);
            grd.addColorStop(0, this.options.shadowColor);
            grd.addColorStop(0.12, this.options._orgStrokeColor);
        ...
        Donut2.prototype.setOptions = function(options) {
            var h, start, stop, w;
            if (options == null) {
                options = null;
            }
            Donut2.__super__.setOptions.call(this, options);
            w = this.canvas.width / 2;
            h = this.canvas.height / 2;
            start = this.radius - this.lineWidth / 2;
            stop = this.radius + this.lineWidth / 2;
